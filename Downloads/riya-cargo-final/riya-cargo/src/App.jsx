...
import { LOGO, IMG_OFFICE, IMG_WH1, IMG_WH2, IMG_TRUCK, IMG_WORKERS, IMG_BOXES, IMG_FURN, IMG_INDOOR } from "./images.js";
...
const G = () => (
  <style>{`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    :root{--r:#e8311a;--rd:#b5240f;--am:#f5a623;--nv:#060d18;--n2:#0c1a2e;--n3:#0f1e30;--w:#f0f2f5;--mu:rgba(240,242,245,0.52);--bd:rgba(255,255,255,0.08)}
    html,body,#root{width:100%;max-width:100vw;overflow-x:hidden}
    html{scroll-behavior:smooth}
    body{font-family:"DM Sans",sans-serif;background:var(--nv);color:var(--w)}
    ...
    .mm{display:none;position:fixed;inset:0;z-index:999;background:rgba(6,13,24,0.98);flex-direction:column;align-items:center;justify-content:center;gap:1.5rem}
    .mm.open{display:flex}
    .mm button{background:transparent;border:none;cursor:pointer;font-family:"Montserrat",sans-serif;font-weight:700;font-size:1.4rem;letter-spacing:2px;text-transform:uppercase;color:rgba(240,242,245,0.7);padding:10px 20px;transition:color 0.2s}
  ...
  return (
    <>
      <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:500,
        background:sc?"rgba(6,13,24,0.97)":"rgba(6,13,24,0.72)",backdropFilter:"blur(16px)",
        borderBottom:sc?"1px solid rgba(232,49,26,0.18)":"1px solid transparent",
...
const Home = ({setPage}) => {
  ...
  useEffect(()=>{const t=setInterval(()=>setSl(s=>(s+1)%3),5500);return()=>clearInterval(t);},[]);
  const slides=[IMG_WORKERS,IMG_FURN,IMG_BOXES];
  const svcs=[
      ...
      <div style={{position:"relative",height:"88vh",minHeight:"500px",maxHeight:"780px",overflow:"hidden"}}>
        ...
        <div className="hero-stats" style={{position:"absolute",bottom:0,right:0,zIndex:5,display:"flex"}}>
          {[["5000+","Moves Done"],["15+","Years Exp"],["100%","Safe Delivery"]].map(([n,l],i)=>(
            <div key={i} style={{background:"rgba(6,13,24,0.88)",backdropFilter:"blur(10px)",
      ...
      <div style={{background:"var(--nv)",padding:"5rem 2rem"}}>
        <div style={{maxWidth:"1200px",margin:"0 auto"}}>
          ...
          <div className="g3" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"1.1rem"}}>
            {svcs.map((s,i)=>(
              <div key={i} style={{background:"rgba(12,26,46,0.6)",border:"1px solid var(--bd)",padding:"1.8rem 1.4rem",borderRadius:"8px",transition:"all 0.3s",cursor:"pointer"}}
        ...
        <div style={{maxWidth:"1100px",margin:"0 auto"}}>
          ...
          <div className="g3" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"1.4rem"}}>
            {[{nm:"Rahul Kale",rt:"Pune to Mumbai",tx:"Every item packed with care, delivered without a scratch. Best movers in Pune!",st:5,in:"RK",cl:"var(--r)"},
              ...
              <div key={i} style={{background:"rgba(6,13,24,0.6)",border:"1px solid var(--bd)",padding:"1.8rem",borderRadius:"8px",borderTop:`3px solid ${t.cl}`,transition:"transform 0.3s"}}
          ...
          <div style={{background:"rgba(12,26,46,0.7)",border:"1px solid var(--bd)",padding:"2.5rem",borderRadius:"12px"}}>
            ...
            {EJS_SVC==="YOUR_SERVICE_ID"&&(
              <div style={{background:"rgba(245,166,35,0.1)",border:"1px solid rgba(245,166,35,0.3)",padding:"0.8rem 1rem",borderRadius:"6px",marginBottom:"1.2rem",...B,fontSize:"0.78rem",color:"var(--am)",lineHeight:1.6}}>
                ⚙️ <strong>Setup EmailJS</strong> to get form submissions in your Gmail. See instructions at top of App.jsx.
              </div>
            )}
            {st==="success"?(
